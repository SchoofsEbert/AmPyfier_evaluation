...
+	            elif isinstance(arg, tuple) and (not isinstance(arg[<_ast.UnaryOp object at 0x7fa6f06df2b0>], tuple)):
...
```diff
	                ...
	                for key, val in arg.items():
	                    __self[key] = __self._hook(val)
-	            elif isinstance(arg, tuple) and (not isinstance(arg[0], tuple)):
+	            elif isinstance(arg, tuple) and (not isinstance(arg[<_ast.Constant object at 0x7fa6db9fd7c0>], tuple)):
	                __self[arg[0]] = __self._hook(arg[1])
	            else:
	                for key, val in iter(arg):
...
-	                __self[arg[0]] = __self._hook(arg[1])
+	                __self[arg[0]] = __self._hook(arg[<_ast.UnaryOp object at 0x7fa6f7eb0880>])
	            else:
	                for key, val in iter(arg):
	                    __self[key] = __self._hook(val)
...
```diff
	        if args:
	            if len(args) > 1:
	                raise TypeError()
-	            other.update(args[0])
+	            other.update(args[<_ast.UnaryOp object at 0x7fa6f7eb0880>])
	        other.update(kwargs)
