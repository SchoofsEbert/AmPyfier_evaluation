...
```diff
	        ...
	        if match:
-	            nhost = match.groups()[0]
+	            nhost = match.groups()[<_ast.UnaryOp object at 0x7f9bb8881c10>]
	            # if the whois address is domain.tld/something then
	            # s.connect((hostname, 43)) does not work
	            if nhost.count('/') > 0:
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.UnaryOp object at 0x7f9bb3a4ab50>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
	            return NICClient.AI_HOST
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.Constant object at 0x7f9bb3f9eeb0>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
	            return NICClient.AI_HOST
...
```
### test_unicode_domain_and_tld_call_add_0_numb_sub_0_none_4
...
```
### test_simple_ascii_domain_call_dup_0_str_hlf_0_str_dbl_0
