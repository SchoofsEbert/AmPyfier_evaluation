...
### test_unicode_domain_and_tld_str_dbl_0_call_dup_0
...
```diff
	        ...
	        if match:
-	            nhost = match.groups()[0]
+	            nhost = match.groups()[<_ast.UnaryOp object at 0x7f6d544fd610>]
	            # if the whois address is domain.tld/something then
	            # s.connect((hostname, 43)) does not work
	            if nhost.count('/') > 0:
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.UnaryOp object at 0x7f6d55e130d0>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.Constant object at 0x7f6d55e13850>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
	            return NICClient.AI_HOST
...
```
### test_unicode_domain_and_tld_str_dbl_0_str_dbl_0_call_dup_0
...
```
### test_unicode_domain_and_tld_str_dbl_0_call_dup_0_call_dup_0
