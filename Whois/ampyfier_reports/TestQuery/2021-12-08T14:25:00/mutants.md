...
### test_unicode_domain_and_tld_call_add_0_none_1_none_1
...
```diff
	        ...
	        if match:
-	            nhost = match.groups()[0]
+	            nhost = match.groups()[<_ast.UnaryOp object at 0x7f2b07abf6d0>]
	            # if the whois address is domain.tld/something then
	            # s.connect((hostname, 43)) does not work
	            if nhost.count('/') > 0:
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.UnaryOp object at 0x7f2b07abf940>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.Constant object at 0x7f2b07abf7c0>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
	            return NICClient.AI_HOST
...
```
### test_simple_unicode_domain_call_dup_0_call_add_1_numb_add_0
...
```
### test_simple_unicode_domain_call_dup_0_call_rem_0
...
```diff
	                ...
	                if len(values) == 1:
-	                    values = values[0]
+	                    values = values[<_ast.UnaryOp object at 0x7f2b0835ba00>]
	                elif not values:
	                    values = None
...
```diff
	    ...
	    url = re.sub('^.*://', '', url)
-	    url = url.split('/')[0].lower()
+	    url = url.split('/')[<_ast.UnaryOp object at 0x7f2b092894f0>].lower()
