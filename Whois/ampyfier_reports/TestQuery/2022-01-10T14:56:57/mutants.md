...
```diff
	        ...
	        if match:
-	            nhost = match.groups()[0]
+	            nhost = match.groups()[<_ast.Constant object at 0x7fae8fbe92b0>]
	            # if the whois address is domain.tld/something then
	            # s.connect((hostname, 43)) does not work
	            if nhost.count('/') > 0:
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.Constant object at 0x7fae8fb8d0a0>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
...
```diff
	        ...
	        tld = domain[-1]
-	        if tld[0].isdigit():
+	        if tld[<_ast.Constant object at 0x7fae8fbe9d90>].isdigit():
	            return NICClient.ANICHOST
	        elif tld == 'ai':
	            return NICClient.AI_HOST
...
```
### test_simple_unicode_domain_call_dup_0_call_add_0_str_dbl_0
...
```
### test_simple_unicode_domain_call_dup_0_str_dbl_0
...
```
### test_unicode_domain_and_tld_call_add_0_none_1_str_emp_1
...
```
### test_unicode_domain_and_tld_str_dbl_0_str_dbl_0_call_add_0
