...
from whois.parser import WhoisEntry, cast_date, WhoisCl, WhoisAr, WhoisBy, WhoisCa, WhoisBiz, WhoisCr, WhoisDe, WhoisNl
...
class TestParser(unittest.TestCase):
    ...
    def test_ca_parse(self):
        ...
        """
        expected_results = {'updated_date': '2016-04-29 00:00:00',
            'registrant_name': 'Test Industries', 'fax': ['+1.123434123',
            '+1.12312993873'], 'dnssec': 'Unsigned', 'registrant_number':
            '70', 'expiration_date': '2020-03-08 00:00:00', 'domain_name':
            'testdomain.ca', 'creation_date': '2000-11-20 00:00:00',
            'phone': ['+1.1235434123x123', '+1.09876545123'], 'status':
            'registered', 'emails': ['testperson1@testcompany.ca',
    ...
    def test_cn_parse(self):
        data = """
            ...
            DNSSEC: unsigned
        """
        expected_results = {'domain_name': 'cnnic.com.cn', 'registrar':
            '北京新网数码信息技术有限公司', 'creation_date': '2000-09-14 00:00:00',
            'expiration_date': '2023-08-16 16:26:39', 'name_servers': [
            'a.cnnic.cn', 'b.cnnic.cn', 'c.cnnic.cn', 'd.cnnic.cn',
            'e.cnnic.cn'], 'status': ['serverDeleteProhibited',
            'serverUpdateProhibited', 'serverTransferProhibited'], 'emails':
            'servicei@cnnic.cn', 'dnssec': 'unsigned', 'name': '中国互联网络信息中心'}
    ...
    def test_il_parse(self):
        ...
        """
        expected_results = {'updated_date': None, 'registrant_name':
            'Arik Baratz', 'fax': None, 'dnssec': 'unsigned',
            'expiration_date': '2018-05-10 00:00:00', 'domain_name':
            'python.org.il', 'creation_date': None, 'phone': [
            '+1 650 6441973', '+1 650 9635533'], 'status':
            'Transfer Locked', 'emails': 'hostmaster@arik.baratz.org',
            'name_servers': ['dns1.zoneedit.com', 'dns2.zoneedit.com',
            'dns3.zoneedit.com'], 'registrar': 'LiveDns Ltd',
...
DNSSEC: signedDelegation
        """
        aexpected_results = {'domain_name': 'rte.ie', 'description': [
            'RTE Commercial Enterprises Limited',
            'Body Corporate (Ltd,PLC,Company)', 'Corporate Name'], 'source':
            'IEDR', 'creation_date': '2000-02-11 00:00:00',
            'expiration_date': '2024-03-31 00:00:00', 'name_servers': [
            ...
            'ns4.rte.ie 162.159.3.18 2400:cb00:2049:1::a29f:312'], 'status':
            'Active', 'admin_id': ['AWB910-IEDR', 'JM474-IEDR'], 'tech_id':
            'JM474-IEDR'}
        expected_results = {'status': 'ok https://icann.org/epp#ok',
            'expiration_date': '2025-03-31 13:20:07', 'creation_date':
            '2000-02-11 00:00:00', 'domain_name': 'rte.ie', 'tech_id':
            '3159-IEDR', 'registrar': 'Blacknight Solutions',
            'name_servers': ['ns1.rte.ie', 'ns2.rte.ie', 'ns3.rte.ie',
            'ns4.rte.ie'], 'admin_id': '202753-IEDR', 'registrar_contact':
            'abuse@blacknight.com'}
    ...
    def test_nl_parse(self):
        ...
        """
        expected_results = {'domain_name': 'utwente.nl', 'name_servers': [
            'ns1.utwente.nl', 'ns2.utwente.nl', 'ns3.utwente.nl'], 'status':
            'active', 'registrar_address': 'Drienerlolaan 5', 'registrar':
            'Universiteit Twente', 'registrar_zip_code': '7522NB',
            'registrar_city': 'ENSCHEDE', 'registrar_country':
            'Netherlands', 'dnssec': 'yes'}
...
Hostname:             p.nic.dk
"""
        expected_results = {'domain_name': 'dk-hostmaster.dk',
            'name_servers': ['auth01.ns.dk-hostmaster.dk',
            'auth02.ns.dk-hostmaster.dk', 'p.nic.dk'], 'status': 'Active',
            'registrant_name': 'DK HOSTMASTER A/S', 'registrant_address':
            'Ørestads Boulevard 108, 11.', 'registrant_zip_code': '2300',
            'registrant_city': 'København S', 'registrant_country': 'DK',
            'dnssec': 'Signed delegation'}
    ...
    def _parse_and_compare(self, domain_name, data, expected_results,
        ...
        for key in expected_results:
            ...
            if expected != result:
                print('%s \t(%s):\t %s != %s' % (domain_name, key, result,
                    expected))
                fail += 1
        if fail:
            self.fail(
    ...
    def test_sk_parse(self):
        ...
        Created:                      2017-11-22
        Updated:                      2017-11-22"""
        expected_results = {'admin': 'H410977', 'admin_city': 'Kosice',
            'admin_country_code': 'SK', 'admin_email': 'info@pipoline.com',
            'admin_organization': 'Pipoline s.r.o', 'admin_postal_code':
            '04012', 'admin_street': 'Ladozska 8', 'creation_date':
            '2012-07-23 00:00:00', 'domain_name': 'pipoline.sk',
            'expiration_date': '2021-07-13 00:00:00', 'name_servers': [
            'ns1.cloudlikeaboss.com', 'ns2.cloudlikeaboss.com'],
            'registrar': 'Pipoline s.r.o.', 'registrar_city': 'Košice',
            'registrar_country_code': 'SK', 'registrar_created':
            '2012-07-23', 'registrar_email': 'peter.gonda@pipoline.com',
            'registrar_name': 'Pipoline s.r.o.',
            'registrar_organization_id': '48273317', 'registrar_phone':
            '+421.949347169', 'registrar_postal_code': '040 12',
            'registrar_street': 'Ladožská 8', 'registrar_updated':
            '2020-07-02', 'updated_date': '2020-07-02 00:00:00'}
        self._parse_and_compare('pipoline.sk', data, expected_results)

    def test_com_expiration_none_2_str_emp_1(self):
        data = """
        Status: ok
        Updated Date: 2017-03-31T07:36:34Z
        Creation Date: 2013-02-21T19:24:57Z
        ...
        >>> Last update of whois database: Sun, 31 Aug 2008 00:18:23 UTC <<<
        """
        w = WhoisEntry.load('', data)

    def test_com_expiration_str_hlf_0_str_dbl_2_str_hlf_0(self):
        data = """13-02-21T19:24:57Z
        Registry Expiry Date: 2018-02-21T1"""
        w = WhoisEntry.load('urlowl.com', data)
        self.assertEqual(w, {'domain_name': None, 'registrar': None,
            'whois_server': None, 'referral_url': None, 'updated_date':
            None, 'creation_date': None, 'expiration_date': '2018-02-21T1',
            'name_servers': None, 'status': None, 'emails': None, 'dnssec':
            None, 'name': None, 'org': None, 'address': None, 'city': None,
            'state': None, 'zipcode': None, 'country': None})
        with self.assertRaises(AttributeError) as excep_info:
            expires = w.expiration_date.strftime('%Y-%m-%d%Y-%m-%d')
        self.assertEqual(excep_info.exception.args, (
            "'str' object has no attribute 'strftime'",))

    def test_com_expiration_str_dbl_1_str_hlf_0(self):
        data = """  Updated Date: 2017-03-31T07:36:34Z
        Creation Date: 2013-02-21T19:24:57Z
        Registry Expiry Date: 2018-02-21T"""
        w = WhoisEntry.load('urlowl.comurlowl.com', data)
        expires = w.expiration_date.strftime('%Y-%m-%d')
        self.assertEqual(expires, '2018-02-21')

    def test_com_expiration_str_hlf_0_str_hlf_0(self):
        data = """       Creation Date: 2013-02-21T19:24:57Z
        Registry E"""
        w = WhoisEntry.load('urlowl.com', data)
        with self.assertRaises(AttributeError) as excep_info:
            expires = w.expiration_date.strftime('%Y-%m-%d')
    ...
    def test_com_expiration_str_emp_0_str_hlf_0(self):
        data = ''
        w = WhoisEntry.load('urlow', data)
        self.assertEqual(w, {'domain_name': None, 'registrar': None,
            'whois_server': None, 'referral_url': None, 'updated_date':
            None, 'creation_date': None, 'expiration_date': None,
            'name_servers': None, 'status': None, 'emails': None, 'dnssec':
            None, 'name': None, 'org': None, 'address': None, 'city': None,
            'state': None, 'zipcode': None, 'country': None})
        with self.assertRaises(AttributeError) as excep_info:
    ...
    def test_com_expiration_str_hlf_1(self):
        data = """
        Status: ok
        Updated Date: 2017-03-31T07:36:34Z
        Creation Date: 2013-02-21T19:24:57Z
        ...
        >>> Last update of whois database: Sun, 31 Aug 2008 00:18:23 UTC <<<
        """
        w = WhoisEntry.load('rlowl', data)
        expires = w.expiration_date.strftime('%Y-%m-%d')
        self.assertEqual(expires, '2018-02-21')

    def test_com_expiration_none_0(self):
        data = None
        with self.assertRaises(AttributeError) as excep_info:
            w = WhoisEntry.load('urlowl.com', data)
        self.assertEqual(excep_info.exception.args, (
    ...
    def test_com_expiration_str_emp_2_str_hlf_0_str_emp_1(self):
        data = """y Expiry Date: 2018-02-21T19:24:57Z

        >>> Last update of whois database: Sun, 31 Aug 2008 00:18:23 UTC <<<
        """
        w = WhoisEntry.load('', data)
        expires = w.expiration_date.strftime('')
        self.assertEqual(expires, '')

    def test_cn_parse_str_hlf_0(self):
        data = """n Status: serverDeleteProhibited
            Domain Status: serverUpdateProhibited
            Domain Status: serverTransferProhibited
            Registrant ID: s1255673574881
            Registrant: 中国互联网络信息中心
        ...
        expected_results = {'domain_name': 'cnnic.com.cn', 'registrar':
            ...
            'serverUpdateProhibited', 'serverTransferProhibited'], 'emails':
            'servicei@cnnic.cn', 'dnssec': 'unsigned', 'name': '中国互联网络信息中心'}
        with self.assertRaises(AssertionError) as excep_info:
            self._parse_and_compare('cnnic.com.cn', data, expected_results)
        self.assertEqual(excep_info.exception.args, (
            '5/9 sample whois attributes were not parsed properly!',))
