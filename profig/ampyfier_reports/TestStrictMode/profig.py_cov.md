...
-	        return d
	    ...
	    def section(self, key, create=None):
	        ...
	        *create* will default to `False` when in strict mode. Otherwise it
	        ...
	        if key is None:
+	            raise InvalidSectionError(key)	#test_set_init_none_2
	        section = self
...
+	        raise NoValueError(self.key)	#test_set_init_str_hlf_1
...
-	            return value
	    ...
	    def convert(self, string, decode=True):
	        """str -> value"""
+	        if self._root.coercer:	#test_clear_uninit_on_sync_call_dup_0
+	            type = self._type	#test_clear_uninit_on_sync_call_dup_0
	            # if we are converting a byte-string and the type is not bytes,
	            # then we need to decode it
+	            if decode and isinstance(string, bytes) and not (	#test_clear_uninit_on_sync_call_dup_0
	                inspect.isclass(type) and issubclass(type, bytes)
	                ):
+	                string = string.decode(self._root.encoding)	#test_clear_uninit_on_sync_call_dup_0
+	            value = self._root.coercer.convert(string, type)	#test_clear_uninit_on_sync_call_dup_0
	        else:
-	            value = string
+	        self.set_value(value)	#test_clear_uninit_on_sync_call_dup_0
...
+	                key.extend(p)	#test_set_init_str_hlf_0
	            elif p is None:
	                pass
	            else:
...
+	            raise ValueError('invalid error_mode: {}'.format(mode))	#test_read_uninit_str_dbl_0
...
+	                section.convert(value, decode=True)	#test_clear_uninit_on_sync_call_dup_0
+	                section._dirty = False	#test_clear_uninit_on_sync_call_dup_0
	            if key in comments:
...
+	            self.write_section(cfg, section, file, first)	#test_clear_uninit_on_sync_none_0
+	            seen.add(section.key)	#test_clear_uninit_on_sync_none_0
+	            first = False	#test_clear_uninit_on_sync_none_0
...
+	        try:	#test_clear_uninit_on_sync_call_dup_0
+	            func = self._converters[self._typename(type)]	#test_clear_uninit_on_sync_call_dup_0
-	        except KeyError:
-	            err = "no converter for: {}"
-	            raise NotRegisteredError(err.format(type))
	
+	        try:	#test_clear_uninit_on_sync_call_dup_0
+	            return func(value)	#test_clear_uninit_on_sync_call_dup_0
-	        except Exception as e:
