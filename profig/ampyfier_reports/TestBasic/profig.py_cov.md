...
-	        return self.valid or len(self) > 0
	    ...
	    def section(self, key, create=None):
	        ...
	        *create* will default to `False` when in strict mode. Otherwise it
	        ...
	        if key is None:
+	            raise InvalidSectionError(key)	#test_root_none_0
	        section = self
...
-	        return sources, format
	    ...
	    def _process_format(self, format):
	        ...
	        """
	        if not format:
+	            return self._format	#test_formats_str_emp_2
...
+	            self._type = None	#test_reset_str_dbl_0
...
+	        except KeyError:	#test_init_value_str_dbl_3
+	            err = "no converter for: {}"	#test_init_value_str_dbl_3
+	            raise NotRegisteredError(err.format(type))	#test_init_value_str_dbl_3
