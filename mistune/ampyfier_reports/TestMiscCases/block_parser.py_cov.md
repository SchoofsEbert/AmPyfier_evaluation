...
+	        if depth > self.BLOCK_QUOTE_MAX_DEPTH - 1:	#test_escape_html_none_4_str_hlf_0
-	            rules = list(self.block_quote_rules)
-	            rules.remove('block_quote')
-	            return rules
+	        return self.block_quote_rules	#test_escape_html_none_4_str_hlf_0
	
	    def parse_block_quote(self, m, state):
+	        depth = state.get('block_quote_depth', 0) + 1	#test_escape_html_none_4_str_hlf_0
+	        state['block_quote_depth'] = depth	#test_escape_html_none_4_str_hlf_0
	
	        # normalize block quote text
+	        text = _BLOCK_QUOTE_LEADING.sub('', m.group(0))	#test_escape_html_none_4_str_hlf_0
+	        text = expand_leading_tab(text)	#test_escape_html_none_4_str_hlf_0
+	        text = _BLOCK_QUOTE_TRIM.sub('', text)	#test_escape_html_none_4_str_hlf_0
+	        text = cleanup_lines(text)	#test_escape_html_none_4_str_hlf_0
	
+	        rules = self.get_block_quote_rules(depth)	#test_escape_html_none_4_str_hlf_0
+	        children = self.parse(text, state, rules)	#test_escape_html_none_4_str_hlf_0
+	        state['block_quote_depth'] = depth - 1	#test_escape_html_none_4_str_hlf_0
+	        return {'type': 'block_quote', 'children': children}	#test_escape_html_none_4_str_hlf_0
...
-	            return {'type': 'block_text', 'text': text.strip()}
	    ...
	    def _iter_render(self, tokens, inline, state):
	        for tok in tokens:
	            ...
	            if 'children' in tok:
+	                children = self.render(tok['children'], inline, state)	#test_escape_html_none_4_str_hlf_0
	            elif 'raw' in tok:
	                children = tok['raw']
	            else:
