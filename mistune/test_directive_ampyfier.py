...
class TestPluginAdmonition(TestCase):
    ...
    def test_unsupported_directive_none_1_str_hlf_0_str_dbl_0(self):
        md = create_markdown(plugins=[Admonition()])
        s = ':: Warnning\n\n  :: Warnning\n\n  '
        html = md(s)

    def test_unsupported_directive_none_1_str_hlf_0(self):
        md = create_markdown(plugins=[Admonition()])
        s = ':: Warnning\n\n  '
        html = md(s)

    def test_unsupported_directive_none_1_str_emp_0(self):
        md = create_markdown(plugins=[Admonition()])
        s = ''
        html = md(s)

    def test_unsupported_directive_none_1_none_0(self):
        md = create_markdown(plugins=[Admonition()])
        s = None
        html = md(s)

    def test_code_admonition_str_hlf_0_none_3_none_2(self):
        md = create_markdown(plugins=[Admonition()])
        s = 'ning\n\n       prin'
        html = md(s)
        self.assertFalse('class="admonition note"' in html)
