...
class TestMarkdownBasics(unittest.TestCase):
    ...
    def testDotNotationExtension_str_emp_1(self):
        """ Test Extension loading with Name (`path.to.module`). """
        with self.assertRaises(ValueError) as excep_info:
            markdown.Markdown(extensions=[''])
        self.assertEqual(excep_info.exception.args, ('Empty module name',))

    def testDotNotationExtension_str_dbl_1(self):
        """ Test Extension loading with Name (`path.to.module`). """
        with self.assertRaises(ModuleNotFoundError) as excep_info:
            markdown.Markdown(extensions=[
                'markdown.extensions.footnotesmarkdown.extensions.footnotes'])
        self.assertEqual(excep_info.exception.args, (
            'Failed loading extension "markdown.extensions.footnotesmarkdown.extensions.footnotes".'
            ,))
        self.assertEqual(excep_info.exception.msg,
            "No module named 'markdown.extensions.footnotesmarkdown'")
        self.assertEqual(excep_info.exception.name,
            'markdown.extensions.footnotesmarkdown')
        self.assertIsNone(excep_info.exception.path)

    def testDotNotationExtension_str_emp_1_str_chr_0_str_dbl_1(self):
        """ Test Extension loading with Name (`path.to.module`). """
        with self.assertRaises(AttributeError) as excep_info:
            markdown.Markdown(extensions=['uu'])
        self.assertEqual(excep_info.exception.args, (
            "Failed to initiate extension 'uu': module 'uu' has no attribute 'makeExtension'"
            ,))

    def testDotNotationExtension_none_1(self):
        """ Test Extension loading with Name (`path.to.module`). """
        markdown.Markdown(extensions=[None])
...
class TestConvertFile(unittest.TestCase):
    ...
    def testFileNames_none_1_str_emp_0(self):
        infile, outfile, outfd = self.getTempFiles('')
        markdown.markdownFromFile(input=infile, output=outfile)
...
class TestBlockParser(unittest.TestCase):
    ...
    def testParseChunk_str_emp_2(self):
        """ Test BlockParser.parseChunk. """
        root = etree.Element('div')
        text = ''
        self.parser.parseChunk(root, text)
    ...
    def testParseChunk_none_1(self):
        """ Test BlockParser.parseChunk. """
    ...
    def testParseDocument_str_chr_0_str_emp_1(self):
        from xml.etree.ElementTree import ElementTree
        """ Test BlockParser.parseDocument. """
        lines = ['', 'y', 'bar', '', '    baz']
        tree = self.parser.parseDocument(lines)
        self.assertIsInstance(tree, ElementTree)
        etree.iselement(tree.getroot())
        self.assertEqual(markdown.serializers.to_xhtml_string(tree.getroot(
            )), """<div><p>y
bar</p><pre><code>baz
</code></pre></div>""")

    def testParseDocument_str_chr_0(self):
        from xml.etree.ElementTree import ElementTree
        """ Test BlockParser.parseDocument. """
        lines = ['#foo', 'y', 'bar', '', '    baz']
        tree = self.parser.parseDocument(lines)
...
class TestHtmlStash(unittest.TestCase):
    ...
    def testSimpleStore_amp(self):
        """ Test HtmlStash.store. """
        self.assertEqual(self.placeholder, '\x02wzxhzdk:0\x03')
        self.assertEqual(self.stash.get_placeholder(0), '\x02wzxhzdk:0\x03')
        self.assertEqual(self.stash.html_counter, 1)
        self.assertEqual(self.stash.rawHtmlBlocks, ['foo'])
...
class TestErrors(unittest.TestCase):
    ...
    def testLoadExtensionFailure(self):
        ...
        self.assertRaises(ImportError, markdown.Markdown, extensions=[
            'non_existant_ext'])

    def testLoadBadExtension(self):
        """ Test loading of an Extension with no makeExtension function. """
        self.assertRaises(AttributeError, markdown.Markdown, extensions=[
            'markdown.util'])
    ...
    def testDotNotationExtensionWithBadClass(self):
        ...
        self.assertRaises(AttributeError, markdown.Markdown, extensions=[
            'markdown.extensions.footnotes:MissingExtension'])

    def testBaseExtention(self):
        """ Test that the base Extension class will raise NotImplemented. """
        self.assertRaises(NotImplementedError, markdown.Markdown,
            extensions=[markdown.extensions.Extension()])

    def testBadOutputFormat_amp(self):
        """ Test failure on bad output_format. """
        markdown.Markdown()
...
class testETreeComments(unittest.TestCase):
    ...
    def testCommentPrettify(self):
        ...
        pretty = markdown.treeprocessors.PrettifyTreeprocessor(markdown.
            Markdown())
...
class testElementTailTests(unittest.TestCase):
    ...
    def setUp(self):
        self.pretty = markdown.treeprocessors.PrettifyTreeprocessor(markdown
            .Markdown())
    ...
    def testBrTailNoNewline_none_1_str_dbl_1(self):
        """ Test that last <br> in tree has a new line tail """
        root = etree.Element(None)
        br = etree.SubElement(root, 'brbr')
        self.pretty.run(root)
...
escaping"></script></div>tail text"""
    ...
    def testProsessingInstruction(self):
        ...
        self.assertEqual(markdown.serializers.to_xhtml_string(pi),
            """<?foo &lt;&amp;"test
escaping"&gt;?>""")
    ...
    def testQNameTag(self):
        ...
        ann.text = 'x^2'
        self.assertEqual(markdown.serializers.to_xhtml_string(div),
            '<div><math display="block" xmlns="http://www.w3.org/1998/Math/MathML"><semantics><msup><mi>x</mi><mn>2</mn></msup><annotations>x^2</annotations></semantics></math></div>'
            )
    ...
    def testRegisterSerializer(self):
        self.assertEqual(markdown.markdown('baz', extensions=[self.
            buildExtension()], output_format='fake'), '<p>foo</p>')
...
bar</p>""")
    ...
    def testRegisterSerializer_str_emp_0(self):
        self.assertEqual(markdown.markdown('', extensions=[self.
            buildExtension()], output_format='fake'), '')

    def testRegisterSerializer_str_dbl_1(self):
        with self.assertRaises(KeyError) as excep_info:
            markdown.markdown('baz', extensions=[self.buildExtension()],
                output_format='fakefake')
        self.assertEqual(excep_info.exception.args, (
            'Invalid Output Format: "fakefake". Use one of "fake", "html", "xhtml".'
            ,))

    def testHTMLOutput_call_add_0_str_hlf_0(self):
        self.assertEqual(markdown.markdown('o  \n', output_format='html'),
            '<p>o  </p>')
...
class testAtomicString(unittest.TestCase):
    ...
    def testString_str_hlf_0_str_emp_1_str_hlf_2(self):
        """at a regular string"""
        tree = etree.Element('')
        p = etree.SubElement(tree, 'p')
        p.text = 'ome *'
    ...
    def testString_str_dbl_3(self):
        """ Test that a regular string is parsed. """
        tree = etree.Element('div')
        p = etree.SubElement(tree, 'p')
        p.text = 'some *text*some *text*'
        new = self.inlineprocessor.run(tree)
        self.assertEqual(markdown.serializers.to_html_string(new),
            '<div><p>some <em>text</em>some <em>text</em></p></div>')

    def testString_none_2_str_emp_2(self):
        """ Test that a regular string is parsed. """
        tree = etree.Element('div')
        p = etree.SubElement(tree, None)
        p.text = ''
        new = self.inlineprocessor.run(tree)
        self.assertEqual(markdown.serializers.to_html_string(new),
            '<div></div>')

    def testString_str_dbl_3_str_dbl_3(self):
        """ Test that a regular string is parsed. """
        tree = etree.Element('div')
        p = etree.SubElement(tree, 'p')
        p.text = 'some *text*some *text*some *text*some *text*'
        new = self.inlineprocessor.run(tree)
        self.assertEqual(markdown.serializers.to_html_string(new),
            '<div><p>some <em>text</em>some <em>text</em>some <em>text</em>some <em>text</em></p></div>'
            )

    def testString_none_3_none_2(self):
        """ Test that a regular string is parsed. """
        tree = etree.Element('div')
        p = etree.SubElement(tree, None)
        p.text = None
    ...
    def testSimpleAtomicString_none_2(self):
        """ Test that a simple AtomicString is not parsed. """
        tree = etree.Element('div')
        p = etree.SubElement(tree, None)
        p.text = markdown.util.AtomicString('some *text*')
...
class TestConfigParsing(unittest.TestCase):
    ...
    def testBooleansParsing(self):
        ...
        self.assertParses(0.0, False)
        self.assertParses('none', False)

    def testPreserveNone(self):
        self.assertIsNone(markdown.util.parseBoolValue('None',
            preserve_none=True))
        self.assertIsNone(markdown.util.parseBoolValue(None, preserve_none=
            True))

    def testInvalidBooleansParsing(self):
        self.assertRaises(ValueError, markdown.util.parseBoolValue, 'novalue')

    def testPreserveNone_str_emp_0(self):
        with self.assertRaises(ValueError) as excep_info:
            markdown.util.parseBoolValue('', preserve_none=True)
        self.assertEqual(excep_info.exception.args, (
            "Cannot parse bool value: ''",))
        markdown.util.parseBoolValue(None, preserve_none=True)

    def testPreserveNone_str_dbl_0(self):
        with self.assertRaises(ValueError) as excep_info:
            markdown.util.parseBoolValue('NoneNone', preserve_none=True)
        self.assertEqual(excep_info.exception.args, (
            "Cannot parse bool value: 'NoneNone'",))
        markdown.util.parseBoolValue(None, preserve_none=True)

    def testPreserveNone_str_hlf_0(self):
        self.assertTrue(markdown.util.parseBoolValue('on', preserve_none=True))
        markdown.util.parseBoolValue(None, preserve_none=True)
...
class TestCliOptionParsing(unittest.TestCase):
    ...
    def setUp(self):
        self.default_options = {'input': None, 'output': None, 'encoding':
            None, 'output_format': 'xhtml', 'lazy_ol': True, 'extensions':
            [], 'extension_configs': {}}
        self.tempfile = ''
    ...
    def testExtensionOption(self):
        options, logging_level = parse_options(['-x',
    ...
    def testMultipleExtensionOptions(self):
        options, logging_level = parse_options(['-x',
            'markdown.extensions.footnotes', '-x',
    ...
    def testExtensionConfigOption(self):
        config = {'markdown.extensions.wikilinks': {'base_url':
            'http://example.com/', 'end_url': '.html', 'html_class': 'test'
    ...
    def textBoolExtensionConfigOption(self):
        config = {'markdown.extensions.toc': {'title': 'Some Title',
            'anchorlink': True, 'permalink': True}}
        self.create_config_file(config)
        options, logging_level = parse_options(['-c', self.tempfile])
        self.default_options['extension_configs'] = config
        self.assertEqual(options, self.default_options)
    ...
    def testQuietOption_amp(self):
        options, logging_level = parse_options(['-q'])
        self.assertEqual((options, logging_level), ({'input': None,
            'output': None, 'extensions': [], 'extension_configs': {},
            'encoding': None, 'output_format': 'xhtml', 'lazy_ol': True}, 60))
        self.assertEqual(logging_level, 60)
        self.assertEqual(CRITICAL, 50)

    def testOutputFileOption_str_hlf_0(self):
        options, logging_level = parse_options(['-', 'foo.html'])
        self.assertEqual((options, logging_level), ({'input': '-', 'output':
            None, 'extensions': [], 'extension_configs': {}, 'encoding':
            None, 'output_format': 'xhtml', 'lazy_ol': True}, 50))
        self.default_options['output'] = 'foo.html'
        self.assertEqual(options, {'input': '-', 'output': None,
            'extensions': [], 'extension_configs': {}, 'encoding': None,
            'output_format': 'xhtml', 'lazy_ol': True})
        self.assertEqual(self.default_options, {'input': None, 'output':
            'foo.html', 'encoding': None, 'output_format': 'xhtml',
            'lazy_ol': True, 'extensions': [], 'extension_configs': {}})
...
class TestAncestorExclusion(unittest.TestCase):
    ...
    class AncestorExtension(markdown.Extension):
        ...
        def extendMarkdown(self, md):
            ...
            pattern = '(\\+)([^\\+]+)\\1'
            md.inlinePatterns.register(TestAncestorExclusion.
                AncestorExample(pattern, 'strong'), 'ancestor-test', 0)

    def setUp(self):
        """Setup markdown object."""
        self.md = markdown.Markdown(extensions=[TestAncestorExclusion.
            AncestorExtension()])
    ...
    def test_ancestors(self):
        ...
        result = (
            '<p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p>'
            )
        self.md.reset()
    ...
    def test_ancestors_tail(self):
        ...
        result = (
            '<p><a href="http://test.com"><strong><em>+em+</em>+strong+</strong></a></p>'
            )
        self.md.reset()
        self.assertEqual(self.md.convert(test), result)

    def test_ancestors_none_2_str_hlf_1(self):
        """ Test that an extension can exclude parent tags. """
        test = ' a [+link+](http://tes'
        result = None
        self.md.reset()
        self.assertEqual(self.md.convert(test),
            '<p>a [<strong>link</strong>](http://tes</p>')

    def test_ancestors_str_dbl_2_str_hlf_1(self):
        """ Test that an extension can exclude parent tags. """
        test = '+ and a [+link+](http:'
        result = (
            '<p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p><p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p>'
            )
        self.md.reset()
        self.assertEqual(self.md.convert(test),
            """<ul>
<li>and a [<strong>link</strong>](http:</li>
</ul>""")
        self.assertEqual(result,
            '<p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p><p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p>'
            )

    def test_ancestors_str_hlf_2_str_hlf_1(self):
        """ Test that an extension can exclude parent tags. """
        test = 'me +test+ and a [+link'
        result = ' and a <a href="http://test.com">+link'
        self.md.reset()
        self.assertEqual(self.md.convert(test),
            '<p>me <strong>test</strong> and a [+link</p>')
        self.assertEqual(result, ' and a <a href="http://test.com">+link')

    def test_ancestors_str_emp_1(self):
        """ Test that an extension can exclude parent tags. """
        test = ''
        result = (
            '<p>Some <strong>test</strong> and a <a href="http://test.com">+link+</a></p>'
    ...
    def test_ancestors_str_hlf_1_str_dbl_1_none_2(self):
        """ Test that an extension can exclude parent tags. """
        test = 't+ and a [+link+](httpt+ and a [+link+](http'
        result = None
        self.md.reset()
        self.assertEqual(self.md.convert(test),
            '<p>t<strong> and a [</strong>link<strong>](httpt</strong> and a [<strong>link</strong>](http</p>'
            )

    def test_ancestors_tail_str_hlf_1_none_2(self):
        """ Test that an extension can exclude parent tags when dealing with a tail. """
        test = 'm+*+strong+**](http'
        result = None
        self.md.reset()
        self.assertEqual(self.md.convert(test),
            '<p>m+<em><strong>strong</strong></em>*](http</p>')
