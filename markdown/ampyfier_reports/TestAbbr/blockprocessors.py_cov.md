...
+	            return None	#testSimpleAbbr_str_emp_1
...
+	            theRest = block[1:]	#testSimpleAbbr_str_dbl_2
+	            if theRest:	#testSimpleAbbr_str_dbl_2
	                # Add remaining lines to master blocks for later.
+	                blocks.insert(0, theRest)	#testSimpleAbbr_str_dbl_2
	        sibling = self.lastChild(parent)
	        if (sibling is not None and sibling.tag == 'pre' and
	           len(sibling) and sibling[0].tag == 'code'):
...
-	            sibling[0].text = util.AtomicString(
	...
	class ReferenceProcessor(BlockProcessor):
	    ...
	    def run(self, parent, blocks):
	        ...
	        m = self.RE.search(block)
	        if m:
+	            id = m.group(1).strip().lower()	#testNestedAbbr_str_hlf_2
+	            link = m.group(2).lstrip('<').rstrip('>')	#testNestedAbbr_str_hlf_2
+	            title = m.group(5) or m.group(6)	#testNestedAbbr_str_hlf_2
+	            self.parser.md.references[id] = (link, title)	#testNestedAbbr_str_hlf_2
+	            if block[m.end():].strip():	#testNestedAbbr_str_hlf_2
	                # Add any content after match back to blocks as separate block
-	                blocks.insert(0, block[m.end():].lstrip('\n'))
+	            if block[:m.start()].strip():	#testNestedAbbr_str_hlf_2
	                # Add any content before match back to blocks as separate block
-	                blocks.insert(0, block[:m.start()].rstrip('\n'))
+	            return True	#testNestedAbbr_str_hlf_2
