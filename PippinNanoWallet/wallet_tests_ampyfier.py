...
class TestWalletUtil(unittest.TestCase):
    ...
    async def setUpClassAsync(cls):
        ...
        cls.wallet = Wallet(seed=
            '7474F694061FB3E5813986AEC8A65340B5DEDB4DF94E394CB44489BEA6B21FCD',
            representative=
            'nano_1oa4sipdm679m7emx9npmoua14etkrj1e85g3ujkxmn4ti1aifbu7gx1zrgr'
            , encrypted=False)
        await cls.wallet.save()
        cls.account = Account(wallet=cls.wallet, address=
            'nano_19zdjp6tfhqzcag9y3w499h36nr16ks6gdsfkawzgomrbxs54xaybmsyamza'
            , account_index=0)
        await cls.account.save()
        cls.adhoc_account = AdHocAccount(wallet=cls.wallet, address=
            'nano_1oa4sipdm679m7emx9npmoua14etkrj1e85g3ujkxmn4ti1aifbu7gx1zrgr'
            , private_key=
            '86A3D926AB6BEBAA678C13823D7A92A97CAFAFD277EBF4B54C42C8BB9806EAEE')
    ...
    def removeMockDB(cls):
        try:
            os.remove(Utils.get_project_root().joinpath(pathlib.PurePath(
                'mock.db')))
        except FileNotFoundError:
            print(str(Utils.get_project_root().joinpath(pathlib.PurePath(
                'mock.db'))))
            pass
        try:
            os.remove(Utils.get_project_root().joinpath(pathlib.PurePath(
    ...
    def test_get_representative(self):
        self.assertEqual(self.wallet_util.get_representative(),
            'nano_1oa4sipdm679m7emx9npmoua14etkrj1e85g3ujkxmn4ti1aifbu7gx1zrgr'
            )
    ...
    def test_private_key_derivation(self):
        ...
        self.assertEqual(
            'DD21B99F1A92D7315BF8592F632B05EC77555DFD6A0B2D99C560537CAFC9A13E',
            self.wallet_util.private_key().upper())
        self.assertEqual(
            '86A3D926AB6BEBAA678C13823D7A92A97CAFAFD277EBF4B54C42C8BB9806EAEE',
            self.wallet_util_adhoc.private_key().upper())
